...
import { Content as PythonLifecycle } from "../../../snippets/python/project_lifecycle.md";
import { Content as TypeScriptLifecycle } from "../../../snippets/typescript/project_lifecycle.md";
import { Content as GoLifecycle } from "../../../snippets/go/project_lifecycle.md";
...
These examples create a scratch project and key, revoke the key, then delete the project.
Set `XBERG_CONTROL_PLANE_URL` to the backend origin and `XBERG_CONTROL_PLANE_TOKEN` to an owner
session token, alongside the deployment environment described in the [quickstart](/getting-started/quickstart/).
