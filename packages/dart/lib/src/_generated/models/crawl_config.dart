...
abstract class CrawlConfig with _$CrawlConfig {
  const factory CrawlConfig({
    ...
    @JsonKey(name: 'document_url_depth') int? documentUrlDepth,

    /// Regex patterns — skip URLs matching these
    @JsonKey(name: 'exclude_paths') List<String>? excludePaths,

    /// Also extract the HTML page content as markdown (default: false)
    @JsonKey(name: 'extract_page_content') bool? extractPageContent,

    /// Follow URLs discovered inside extracted documents (default: false)
    @JsonKey(name: 'follow_document_urls') bool? followDocumentUrls,

    /// Regex patterns — only crawl URLs matching these
    @JsonKey(name: 'include_paths') List<String>? includePaths,

    /// Maximum concurrent requests (default: 10)
    @JsonKey(name: 'max_concurrent') int? maxConcurrent,
    ...
    @JsonKey(name: 'max_pages') int? maxPages,
    ...
    @JsonKey(name: 'request_timeout_secs') int? requestTimeoutSecs,

    /// Respect robots.txt (default: true)
    @JsonKey(name: 'respect_robots_txt') bool? respectRobotsTxt,

    /// Stay on the same domain as seed URL (default: true)
    @JsonKey(name: 'stay_on_domain') bool? stayOnDomain,
  ...
  factory CrawlConfig.fromJson(Map<String, Object?> json) =>
